module nf95_gunp_var_m
  ...
  interface nf95_gunp_var
     ...
     module procedure nf95_gunp_var_real_1d, nf95_gunp_var_real_2d, &
          nf95_gunp_var_real_3d, nf95_gunp_var_real_4d, nf95_gunp_var_dble
...
contains
  ...
  subroutine nf95_gunp_var_real_4d(ncid, varid, values, was_packed, start, &
       count_nc, new_missing)
    ...
    if (ncerr_scale == nf95_noerr .or. ncerr_offset == nf95_noerr) then
       ...
       if (all(xtype /= [NF95_BYTE, NF95_SHORT, NF95_INT])) then
          write(error_unit, fmt = *) "nf95_gunp_var_real_4d: type of packed " &
               // "variable must be integer (any kind)"
          stop 1
       end if
  ...
  end subroutine nf95_gunp_var_real_4d
